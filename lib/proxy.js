...
function proxy(graph, options, callback) {
  ...
  const httpWrapper = (httpOptions, callback) => {
    if (httpOptions.url && httpOptions.url.indexOf('httproxy://') === 0) {
      const matched = ruleSearch(options.rules, httpOptions.url.substr(10), httpOptions.method)
      if (matched) {
        proxy(matched, Object.assign({}, options, {
          onstat(requests) {
            httpRequests.push(...requests)
          }
        }), callback)
      }
      else {
        callback(new Error(`no rule for ${httpOptions.url}:${httpOptions.method}`))
      }
      return
    }
